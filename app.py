...
def get_result(result_id):
    api_key = session.get('bfl_api_key')
    url = f"https://api.bfl.ai/v1/get_result?id={result_id}"
    headers = {
        "x-key": api_key
    }
    response = requests.get(url, headers=headers)
...
def generate_bfl_image(prompt, width, height, variant, steps, prompt_upsampling, seed, guidance, safety_tolerance, interval):
    api_key = session.get('bfl_api_key')
    url = "https://api.bfl.ai/v1/image"
    headers = {
        "x-key": api_key,
...
def index():
    ...
    if request.method == 'POST':
        ...
        session['bfl_api_key'] = request.form['bfl_api_key']  # BFL用のAPIキーを保存
        api_key = session['api_key']
        bfl_api_key = session['bfl_api_key']
...
def get_result_route():
    if not session.get('bfl_api_key'):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            return render_template('get_result.html', result=result)
        except Exception as e:
            return str(e)
    return render_template('get_result.html')

@app.route('/bfl_image_generate', methods=['GET', 'POST'])
def bfl_image_generate_route():
    if not session.get('bfl_api_key'):
        return redirect(url_for('index'))
    ...
    if request.method == 'POST':
        ...
        prompt_upsampling = bool(request.form.get('prompt_upsampling', False))
        seed = request.form.get('seed')
        seed = int(seed) if seed else None
        guidance = float(request.form['guidance'])
        safety_tolerance = int(request.form['safety_tolerance'])
        interval = float(request.form['interval'])

        try:
            response = generate_bfl_image(prompt, width, height, variant, steps, prompt_upsampling, seed, guidance, safety_tolerance, interval)
            image_id = response['id']
            image_url = f"https://api.bfl.ai/images/{image_id}.png"  # 仮のURL
            return redirect(url_for('bfl_generated_image', image_id=image_id, seed=seed, image_url=image_url))
        except Exception as e:
            return str(e)
    return render_template('bfl_image_generate.html')

@app.route('/bfl_generated_image')
def bfl_generated_image():
